...
class SlackData(AgentCheck):
    def check(self, instance):
        ...
        data = [
            ('token', '<token_string>'),
            # refactor to run on all channels set in config yaml file
            ('channel', 'C8VH6NR2A'),
        ...
        self.gauge('slack.data.unanswered', unanswered, tags=[
                   'SLACK_CHECK_TEST:test', "channel:general"])
        self.gauge('slack.data.24hr_avg_response_time',
                   avg_response_time_24hr, tags=['channel:general'])
